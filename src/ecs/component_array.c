...
void component_array_init(component_array_t *arr, const size_t component_size) {
    hash_map_init(&arr->entity_to_index, MAX_ENTITIES / 2, sizeof(size_t));
    hash_map_init(&arr->index_to_entity, MAX_ENTITIES / 2, sizeof(entity_t));
...
void component_array_close(component_array_t* const arr) {
